...
const int TILESIZE = 30,
          ...
          TOP = 100,
          HEADERSIZE = 70,
          WIDTH = COL * TILESIZE, HEIGHT = ROW * TILESIZE;
...
void countBombs(tile (&arr)[TILE_COUNT])
{
    ...
    arr[TILE_COUNT - COL].nextTo = arr[TILE_COUNT - COL - COL].bomb + arr[TILE_COUNT - COL + 1].bomb + arr[TILE_COUNT - COL - COL + 1].bomb; // sw corner
    arr[TILE_COUNT - 1].nextTo = arr[TILE_COUNT - 2].bomb + arr[TILE_COUNT - COL - 1].bomb + arr[TILE_COUNT - COL - 2].bomb;                 // se corner
    // top row
    int i, j, ind;
    for (i = 1; i < COL - 1; ++i)
    {
    ...
    // bot row
    for (i = TILE_COUNT - COL + 1; i < TILE_COUNT - 1; ++i)
    {
        arr[i].nextTo = arr[i - 1].bomb + arr[i + 1].bomb + arr[i - COL].bomb + arr[i - COL - 1].bomb + arr[i - COL + 1].bomb;
    }
    // left col
    for (i = COL; i < TILE_COUNT - COL; i += COL)
    {
        arr[i].nextTo = arr[i - COL].bomb + arr[i - COL + 1].bomb + arr[i + 1].bomb + arr[i + COL].bomb + arr[i + COL + 1].bomb;
    }
    // right col
    for (i = COL + COL - 1; i < TILE_COUNT - 1; i += COL)
    {
        arr[i].nextTo = arr[i - COL].bomb + arr[i - COL - 1].bomb + arr[i - 1].bomb + arr[i + COL].bomb + arr[i + COL - 1].bomb;
    }
...
Gdiplus::Color back = Gdiplus::Color(255, 192, 192, 192),
               ...
               seven = Gdiplus::Color(255, 200, 40, 160);

void setDimensions(const HWND &w)
{
...
void updateTiles()
{
    ...
    Gdiplus::SolidBrush behind(black), oneMine(one), twoMine(two), threeMine(three), fourMine(four),
        fiveMine(five), sixMine(six), sevMine(seven), eightMine(black), wiper(back), wiper2(backer);

    Gdiplus::Bitmap bmp(WIDTH, HEIGHT);
    Gdiplus::Graphics *graph = Gdiplus::Graphics::FromImage(&bmp);
    graph->FillRectangle(&wiper, 0, 0, WIDTH, HEIGHT);
    int x, y, ind;
...
void shuffle(int (&shuffledArray)[TILE_COUNT])
{
    srand(time(0));
    int i, ind, temp;
...
void addMines(tile (&arr)[TILE_COUNT], int (&shuffledArray)[TILE_COUNT])
{
...
void setup(tile (&arr)[TILE_COUNT])
{
    addMines(arr, shuffledArray);
    countBombs(arr);
}

void resetTiles(tile (&arr)[TILE_COUNT])
{
    bombs = 99;
...
void revealTile(const int &x, const int &y, const int &ind)
{
    if (arr[ind].hidden)
    {
...
void showNear(const int &x, const int &y)
{
    if (y > 0)
    {
        revealTile(x, y - 1, (30 * (y - 1)) + x);
        if (x > 0)
            revealTile(x - 1, y - 1, (30 * (y - 1)) + x - 1);
        if (x < 29)
            revealTile(x + 1, y - 1, (30 * (y - 1)) + x + 1);
    }
    if (y < 15)
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char *lpCmdLine, int nShowCmd)
{
    ...
    while (active)
    {
        ...
        if (inProgress && hasClicked && tick < (999 * 50))
            ++tick;
        if (PeekMessage(&message, window, 0, 0, PM_REMOVE | PM_NOYIELD))
        { // this while loop processes message for window
